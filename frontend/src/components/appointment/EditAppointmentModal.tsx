...
export const EditAppointmentModal: React.FC<EditAppointmentModalProps> = ({
  ...
  const [formData, setFormData] = useState<UpdateAppointmentRequest>(() => {
    ...
    return {
      ...
      appointmentDate: appointmentDate.toISOString().slice(0, 16), // YYYY-MM-DDTHH:mm format
  ...
  const handleInputChange = (field: keyof UpdateAppointmentRequest, value: any) => {
    setFormData(prev => ({
      ...prev,
      [field]: value,
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Validate appointment date if changed
      ...
      const errorMessage = error.response?.data?.error || '예약 수정에 실패했습니다.';
      alert(errorMessage);
    } finally {
      setLoading(false);
    }
