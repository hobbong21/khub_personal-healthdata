...
exports.RiskCalculationEngine = void 0;
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
class RiskCalculationEngine {
    static async calculateCardiovascularRisk(input) {
        const factors = {
            geneticRisk: 0.5,
            lifestyleRisk: 0.5,
        ...
        if (input.userProfile?.lifestyle) {
            ...
            if (lifestyle.exerciseFrequency < 2)
                lifestyleScore += 0.2;
            if (lifestyle.dietType === 'high_fat' || lifestyle.dietType === 'high_sodium')
                lifestyleScore += 0.15;
            factors.lifestyleRisk = Math.min(lifestyleScore, 1);
        }
        if (input.familyHistory) {
            let familyScore = 0.3;
            const cvdRelatives = input.familyHistory.filter(relative => relative.conditions?.some((condition) => condition.toLowerCase().includes('heart') ||
    ...
    static async calculateDiabetesRisk(input) {
        const factors = {
            geneticRisk: 0.5,
            lifestyleRisk: 0.5,
            familyHistoryRisk: 0.5,
        ...
        if (input.userProfile?.lifestyle) {
            ...
            if (lifestyle.exerciseFrequency < 3)
                lifestyleScore += 0.25;
            if (lifestyle.dietType === 'high_sugar' || lifestyle.dietType === 'processed')
                lifestyleScore += 0.2;
            if (lifestyle.alcohol === 'heavy')
                lifestyleScore += 0.1;
            if (input.userProfile.bmi && input.userProfile.bmi > 30)
                lifestyleScore += 0.2;
            else if (input.userProfile.bmi && input.userProfile.bmi > 25)
                lifestyleScore += 0.1;
            factors.lifestyleRisk = Math.min(lifestyleScore, 1);
        }
        if (input.familyHistory) {
            let familyScore = 0.3;
            const diabetesRelatives = input.familyHistory.filter(relative => relative.conditions?.some((condition) => condition.toLowerCase().includes('diabetes')));
    ...
    static async calculateAlzheimerRisk(input) {
        const factors = {
            geneticRisk: 0.5,
            lifestyleRisk: 0.5,
            familyHistoryRisk: 0.5,
        ...
        if (input.userProfile?.lifestyle) {
            ...
            if (lifestyle.alcohol === 'heavy')
                lifestyleScore += 0.1;
            if (lifestyle.cognitiveActivities === 'low')
                lifestyleScore += 0.1;
            factors.lifestyleRisk = Math.min(lifestyleScore, 1);
        }
        if (input.familyHistory) {
            let familyScore = 0.3;
            const alzheimerRelatives = input.familyHistory.filter(relative => relative.conditions?.some((condition) => condition.toLowerCase().includes('alzheimer') ||
    ...
    static async calculateCancerRisk(input, cancerType) {
        const factors = {
            geneticRisk: 0.5,
            lifestyleRisk: 0.5,
            familyHistoryRisk: 0.5,
        ...
        if (input.userProfile?.lifestyle) {
            ...
            switch (cancerType) {
                ...
                case 'breast':
                    ...
                    if (lifestyle.exerciseFrequency < 2)
                        lifestyleScore += 0.1;
                    if (lifestyle.hormonalFactors?.hrt)
                        lifestyleScore += 0.1;
                    break;
                case 'colorectal':
                    if (lifestyle.dietType === 'high_red_meat')
                        lifestyleScore += 0.15;
                    if (lifestyle.exerciseFrequency < 2)
                        lifestyleScore += 0.1;
                    if (lifestyle.smoking)
        ...
        if (input.familyHistory) {
            let familyScore = 0.3;
            const cancerRelatives = input.familyHistory.filter(relative => relative.conditions?.some((condition) => condition.toLowerCase().includes(cancerType) ||
    ...
    static async calculateIntegratedRisk(input) {
        let riskFactors;
        ...
        const integratedRisk = riskFactors.geneticRisk * weights.genetic +
    ...
    static getWeightsForDisease(diseaseType) {
        const weightMap = {
    ...
    static calculatePercentile(riskScore, diseaseType, userAge, userGender) {
        const populationData = this.getPopulationRiskData(diseaseType, userAge, userGender);
        if (riskScore <= populationData.p10)
            return 10;
        if (riskScore <= populationData.p25)
            return 25;
        if (riskScore <= populationData.p50)
            return 50;
        if (riskScore <= populationData.p75)
            return 75;
        if (riskScore <= populationData.p90)
            return 90;
        return 95;
    }
    static getPopulationRiskData(diseaseType, age, gender) {
        const baseRisks = {
            'cardiovascular_disease': {
        ...
        if (diseaseType === 'cardiovascular_disease' && gender === 'male')
            genderMultiplier = 1.2;
        if (diseaseType === 'breast_cancer' && gender === 'female')
            genderMultiplier = 1.0;
        if (diseaseType === 'prostate_cancer' && gender === 'male')
            genderMultiplier = 1.0;
    ...
    static generateRiskRecommendations(diseaseType, riskFactors, riskScore) {
...
exports.RiskCalculationEngine = RiskCalculationEngine;
//# sourceMappingURL=riskCalculationEngine.js.map