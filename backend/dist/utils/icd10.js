...
exports.SAMPLE_ICD10_CODES = void 0;
exports.validateICD10Code = validateICD10Code;
exports.normalizeICD10Code = normalizeICD10Code;
exports.getICD10Category = getICD10Category;
exports.searchICD10Codes = searchICD10Codes;
exports.getICD10CodeDetails = getICD10CodeDetails;
exports.validateAndSuggestICD10 = validateAndSuggestICD10;
...
function searchICD10Codes(searchTerm, limit = 10) {
    if (!searchTerm || searchTerm.trim().length === 0) {
        return exports.SAMPLE_ICD10_CODES.slice(0, limit);
    }
    const term = searchTerm.toLowerCase().trim();
    return exports.SAMPLE_ICD10_CODES
        .filter(code => code.code.toLowerCase().includes(term) ||
        code.description.toLowerCase().includes(term) ||
        code.category.toLowerCase().includes(term))
        .slice(0, limit);
}
function getICD10CodeDetails(code) {
    const normalizedCode = normalizeICD10Code(code);
    if (!validateICD10Code(normalizedCode)) {
        return null;
    }
    const found = exports.SAMPLE_ICD10_CODES.find(item => item.code === normalizedCode);
    if (found) {
        return found;
    }
    return {
...
function validateAndSuggestICD10(code) {
