...
import { requirePermission, Permission, auditLog } from '../middleware/security';
import { logDataAccess } from '../middleware/monitoring';
...
router.get('/dashboard', 
  ...
  async (req, res) => {
    try {
      ...
      auditLog(req, 'SECURITY_DASHBOARD_ACCESSED', {
...
router.get('/suspicious-patterns',
  requirePermission(Permission.SYSTEM_ADMIN),
  logDataAccess('suspicious_patterns', 'read'),
  async (req, res) => {
    try {
      const patterns = securityAuditService.detectSuspiciousPatterns();

      auditLog(req, 'SUSPICIOUS_PATTERNS_ANALYZED', {
...
router.post('/events/:eventId/resolve',
  requirePermission(Permission.SYSTEM_ADMIN),
  logDataAccess('security_event', 'write'),
  async (req, res) => {
    try {
      ...
      const success = await securityAuditService.resolveSecurityEvent(eventId, resolvedBy);
      ...
      auditLog(req, 'SECURITY_EVENT_RESOLVED', {
        eventId,
        resolvedBy
      });
...
router.get('/events/export',
  ...
  async (req, res) => {
    try {
      ...
      const exportData = securityAuditService.exportSecurityEvents(
        timeRange,
        format as 'json' | 'csv'
      );

      auditLog(req, 'SECURITY_EVENTS_EXPORTED', {
        timeRange,
        format,
...
router.post('/performance/optimize',
  requirePermission(Permission.SYSTEM_ADMIN),
  logDataAccess('performance_optimization', 'write'),
  async (req, res) => {
    try {
      const report = await performanceOptimizationService.generateOptimizationReport();

      auditLog(req, 'PERFORMANCE_OPTIMIZATION_EXECUTED', {
...
router.get('/performance/history',
  requirePermission(Permission.SYSTEM_ADMIN),
  logDataAccess('performance_history', 'read'),
  async (req, res) => {
    try {
      const { limit = 10 } = req.query;
      const history = performanceOptimizationService.getOptimizationHistory(Number(limit));
...
router.get('/performance/trends',
  requirePermission(Permission.SYSTEM_ADMIN),
  logDataAccess('performance_trends', 'read'),
  async (req, res) => {
    try {
      const trends = performanceOptimizationService.analyzePerformanceTrends();
 ...
 * 자동 최적화 설정
 */
router.put('/performance/auto-optimization',
  requirePermission(Permission.SYSTEM_ADMIN),
  logDataAccess('auto_optimization_settings', 'write'),
  async (req, res) => {
    try {
      ...
      performanceOptimizationService.setAutoOptimization(enabled);

      auditLog(req, 'AUTO_OPTIMIZATION_SETTING_CHANGED', {
        enabled,
...
router.get('/health',
  async (req, res) => {
    try {
      const securityDashboard = securityAuditService.getSecurityDashboard();
      const performanceTrends = performanceOptimizationService.analyzePerformanceTrends();
